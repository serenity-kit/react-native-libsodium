...
Download a the new minisig file e.g. `libsodium-1.0.21-stable.tar.gz.minisig` to `./libsodium`. Update the version in `./libsodium/build.sh` and run `cd libsodium && ./build.sh` to build the new library.
